...
public class AutonomousPopulationRegulator implements IBot
{
    ...
    private GameSimulator createSimulator(IGameState state) {
        AutonomousPopulationRegulator.GameSimulator simulator = new AutonomousPopulationRegulator.GameSimulator(new GameState());
        simulator.setGameOver(AutonomousPopulationRegulator.GameOverState.Active);
    ...
    private IMove calculateWinningMove(IGameState state, int maxTimeMs){
        ...
        while (System.currentTimeMillis() < time + maxTimeMs) { // check how much time has passed, stop if over maxTimeMs
            AutonomousPopulationRegulator.GameSimulator simulator = createSimulator(state);
            ...
            while (simulator.getGameOver()== AutonomousPopulationRegulator.GameOverState.Active){ // Game not ended
                simulator.updateGame(randomMovePlayer);

                // Opponent plays randomly
                if (simulator.getGameOver()== AutonomousPopulationRegulator.GameOverState.Active){ // game still going
            ...
            if (simulator.getGameOver()== AutonomousPopulationRegulator.GameOverState.Win){
    ...
    class GameSimulator {
        ...
        private volatile AutonomousPopulationRegulator.GameOverState gameOver = AutonomousPopulationRegulator.GameOverState.Active;

        public void setGameOver(AutonomousPopulationRegulator.GameOverState state) {
            gameOver = state;
        }

        public AutonomousPopulationRegulator.GameOverState getGameOver() {
            return gameOver;
        }
        ...
        private void checkAndUpdateIfWin(IMove move) {
            ...
            if (macroBoard[macroX][macroY].equals(IField.EMPTY_FIELD) ||
                ...
                //Check macro win
                if (isWin(macroBoard, new AutonomousPopulationRegulator.Move(macroX, macroY), "" + currentPlayer))
                    gameOver = AutonomousPopulationRegulator.GameOverState.Win;
                else if (isTie(macroBoard, new AutonomousPopulationRegulator.Move(macroX, macroY)))
                    gameOver = AutonomousPopulationRegulator.GameOverState.Tie;
            }
